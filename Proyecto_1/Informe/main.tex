...
\end{equation}
\begin{equation}
    \dot{\vec{r}}_{1}\cdot\hat{\theta}  = (\dot{\vec{R}} - b\dot{\hat{\theta}})\cdot\hat{\theta} = 0 \quad \longrightarrow \quad \boxed{(\dot{\vec{R}}\cdot\hat{\theta}) = 0}  
\end{equation}
...
\begin{equation}
    2(\dot{\vec{R}}\cdot\hat{n}) = a(\dot\phi_{1} + \dot\phi_{2}) \qquad \longrightarrow \qquad \boxed{(\dot{\vec{R}}\cdot\hat{n}) = a\dot\Phi} ~\nonumber
\end{equation}
\begin{equation}
...
\begin{equation}
    \dot\phi_{1}^{2} + \dot\phi_{2}^{2} = \frac{1}{2}\left[(2\dot\Phi)^{2} + \dot\psi^{2}\right] = \frac{1}{2}\left[(2\dot\Phi)^{2} + \left(\frac{2b}{a}\right)^{2}\dot\theta^{2}\right].~\nonumber
\end{equation}
...
\begin{pmatrix}
    \dot{x} \\
    \dot{y}
\end{pmatrix} = a\left[\frac{a\lambda{F_0}}{I_{m}\dot\theta_{0}}\left(\cos(\theta_{0}) - \cos(\dot\theta_{0}t + \theta_{0})\right) +  \dot\Phi_{0} \right]
\begin{pmatrix}
...
\end{align*}
Estas trayectorias son muy similares al patrón con el que uno usualmente dibuja resortes.
...
\begin{equation}
    \ddot{\Phi}(t) = -\frac{a\lambda{k}}{I_m}(x\cos(\dot\theta_{0}t + \theta_{0}) + y\sin(\dot\theta_{0}t + \theta_{0})) = -\frac{a\lambda{k}}{I_{m}}\left(\vec{R}\cdot\hat{n}\right)~\nonumber
\end{equation}
\begin{equation}
    \dddot\Phi(t) = -\frac{a\lambda{k}}{I_{m}}\left[(\dot{\vec{R}}\cdot\hat{n}) + \dot\theta_{0}(\vec{R}\cdot\hat\theta)\right]~\nonumber
\end{equation}
El primer término está relacionado con $f_{2}$:
\begin{equation}
...
\end{equation}
donde el primer término del paréntesis se anula por ser exactamente $f_1$, y el segundo se puede reescribir considerando que $\dot{\hat{\theta}} = -\dot{\theta}_{0}\hat{n}$, por tanto:
\begin{equation}
    \ddddot\Phi(t) = -\frac{a\lambda{k}}{I_{m}}\left(a - \frac{\dot\theta_{0}I_{m}}{a\lambda{k}}\right)\ddot\Phi(t) = -\left(\frac{a^{2}\lambda{k}}{I_{m}} - \dot\theta_{0}\right)\ddot\Phi(t) = -\Omega^{2}\ddot\Phi(t).~\nonumber
\end{equation}
De esta forma se llega a una ecuación armónica para $\ddot\Phi(t)$, la cual conduce a soluciones de la forma:
\begin{equation}
    \ddot\Phi(t) = \frac{\ddot\Phi_{0}}{\cos(\delta)}\cos(\Omega{t} + \delta) \quad \longrightarrow \quad \dot\Phi(t) = \frac{\ddot\Phi_{0}}{\Omega\cos(\delta)}\sin(\Omega{t} + \delta) - \frac{\ddot\Phi_{0}}{\Omega}\tan(\delta) + \dot\Phi_{0} ~\nonumber
\end{equation}
\begin{equation}
...
\begin{pmatrix}
    \dot{x} \\
    \dot{y}
\end{pmatrix} = \left[\frac{\ddot\Phi_{0}}{\Omega\cos(\delta)}\sin(\Omega{t} + \delta) - \frac{\ddot\Phi_{0}}{\Omega}\tan(\delta) + \dot\Phi_{0}\right]
\begin{pmatrix}
    \cos(\dot\theta_{0}t + \theta_0) \\
    \sin(\dot\theta_{0}t + \theta_0)
\end{pmatrix},
...
con soluciones, para el caso de no resonancia ($\Omega \neq \dot\theta_{0}$):

\begin{align*}
x(t) = x_0 &+ \frac{a}{\dot{\theta}_0} \left( \dot{\Phi}_0 - \frac{\ddot{\Phi}_0 \tan \delta}{\Omega} \right) \left[ \sin(\dot{\theta}_0 t + \theta_0) - \sin \theta_0 \right] \notag \\
&+ \frac{a\ddot{\Phi}_0}{2 \Omega \cos \delta} \left[ \frac{\cos(\delta + \theta_0) - \cos((\Omega + \dot{\theta}_0)t + \delta + \theta_0)}{\Omega + \dot{\theta}_0} + \frac{\cos(\delta - \theta_0) - \cos((\Omega - \dot{\theta}_0)t + \delta - \theta_0)}{\Omega - \dot{\theta}_0} \right] \\[15pt]
y(t) = y_0 &- \frac{a}{\dot{\theta}_0} \left( \dot{\Phi}_0 - \frac{\ddot{\Phi}_0 \tan \delta}{\Omega} \right) \left[ \cos(\dot{\theta}_0 t + \theta_0) - \cos \theta_0 \right] \notag \\
&+ \frac{a\ddot{\Phi}_0}{2 \Omega \cos \delta} \left[ \frac{\sin((\Omega - \dot{\theta}_0)t + \delta - \theta_0) - \sin(\delta - \theta_0)}{\Omega - \dot{\theta}_0} - \frac{\sin((\Omega + \dot{\theta}_0)t + \delta + \theta_0) - \sin(\delta + \theta_0)}{\Omega + \dot{\theta}_0} \right],
\end{align*}

donde las trayectorias muestran estructuras similares a flores polares (aunque más bien serían como mandalas), mientras para el caso resonante ($\Omega = \dot\theta_{0}$):
\begin{align*}
...
\end{align*}
el cual muestra trayecorias abiertas que son muy similares a las del porencial lineal, sólo que en diagonal y con formas menos uniformes.
\end{document}